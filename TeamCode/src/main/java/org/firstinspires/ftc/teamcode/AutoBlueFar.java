...
//
//    private final Pose humanZone = new Pose(5, 8, Math.toRadians(220));
//    private final Pose backPose = new Pose(31.5, 8, Math.toRadians(180));
//
//    private final Pose spike3pose = new Pose(25, 32, Math.toRadians(180));
//    private final Pose controlSpike3 = new Pose(45.58357501607667, 35);
//
...
//                        HeadingInterpolator.facingPoint(blueGoalXInches, blueGoalYInches)
//                )
//                .build();
//
...
public class AutoBlueFar extends OpMode {
    ...
    // ── Timing ────────────────────────────────────────────────────────────────
    public static double waitDurationSeconds   = 0.01;
    public static double maxWaitForShotSeconds = 4;
    public static double gateWaitSeconds       = 0.8;
    public static double rpmTolerance          = 125.0;

    // ── Pulse-based shot parameters ───────────────────────────────────────────
    public static double transferOpenSeconds   = 0.13;
    public static double transferClosedSeconds = 0.32;
    ...
    public static int    targetRPM1     = 3050;
    public static double hoodAngle1Deg  = 48;

    public static int    targetRPM2     = 3050;
    public static double hoodAngle2Deg  = 48;
//    public static double turretShot1Pos = 0.49;
//    public static double turretShot2Pos = 0.4725;

    // ── Poses ─────────────────────────────────────────────────────────────────
    private final Pose startPose           = new Pose(56, 8.9, Math.toRadians(180));
    private final Pose shootPose           = new Pose(56.11586314152411, 13.20373250388803, Math.toRadians(120));
    private final Pose humanZone           = new Pose(8, 10.5, Math.toRadians(180));
    private final Pose backPose            = new Pose(31.5, 11.5, Math.toRadians(180));
    private final Pose spike3pose          = new Pose(25, 33, Math.toRadians(180));
    private final Pose controlSpike3       = new Pose(47, 35);
    private final Pose overflowPick        = new Pose(12.89, 38.12498924965115, Math.toRadians(120));
    private final Pose overflowControlPose = new Pose(18.893564047419593, 1.0286688083403455);
    ...
    public void init() {
        ...
        buildPaths();
        setSequenceState(SequenceState.WAITING);
        stopTransferAction.run();
        turret.commandPosition(Turrettt.safeMiddle);
    }
...
//        updateTurret();
        ...
        telemetry.addData("Sequence",        sequenceState);
        telemetry.addData("Wait Time Left",  sequenceState == SequenceState.WAITING
                ? String.format("%.1fs", Math.max(0, waitDurationSeconds - segmentTimer.seconds())) : "N/A");
        telemetry.addData("Path Busy",       follower.isBusy());
        telemetry.addData("Shot Triggered",  shotTriggered);
        telemetry.addData("Shot Finished",   shotFinished);
        telemetry.addData("Shot Pulses",     shotPulseCount);
        telemetry.addData("Pose X",          follower.getPose().getX());
        telemetry.addData("Pose Y",          follower.getPose().getY());
        telemetry.addData("Pose Heading",    follower.getPose().getHeading());
        telemetry.addData("Segment Time",    segmentTimer.seconds());
...
//    }
    ...
    private void configureSequenceIfNeeded() {
        ...
        if (sequenceState == SequenceState.WAITING) {
            segmentStarted = true;
            return;
        }
    ...
    private void runCurrentSequence() {
        // ── Configurable wait before starting auto ─────────────────────────────
        if (sequenceState == SequenceState.WAITING) {
            if (segmentTimer.seconds() >= waitDurationSeconds) {
                setSequenceState(SequenceState.PRELOAD_SHOT);
            }
            return;
        }
        ...
        if (shootDuringPath) {
            ...
            if (pathDone && !shotTriggered && !shotFinished) {
                ...
                if (flywheelReady) {
                    // ── Open transfer and begin pulse sequence ─────────────────
            ...
            if (shotTriggered && !shotFinished) {
                if (transferCurrentlyOpen && shotTimer.seconds() >= transferOpenSeconds) {
                    ...
                    shotTimer.reset();
                        ...
                        && shotPulseCount >= shotPulseCountTarget
                        && shotTimer.seconds() >= transferClosedSeconds) {
                    // All pulses done — stop intake and mark shot complete
                    intakeAction.stop();
                    shotFinished = true;
                }
            }
...
//                )
    ...
    // =========================================================================
    private boolean isAtSpeed(double targetRPM) {
        return Math.abs(flyWheelSubsystem.getCurrentRPMLeft()  - targetRPM) <= 300
                && Math.abs(flyWheelSubsystem.getCurrentRPMRight() - targetRPM) <= 300;
    }
